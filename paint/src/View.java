 ...
 */
        ...
        */
		...
		public void actionPerformed(ActionEvent event) {
			if (event.getSource() == Line) {
				...
				int code =
	JOptionPane.showOptionDialog(panel, " choose where to save the file", "Save",
   ...
   null, options, options[1]);
		...
		(panel, " choose the file to be loaded", "Load",
JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
				...
				if (panel.getCurrentShapeObject() != null && panel.getEnable()) {
					if (!((Shape2D) panel.getCurrentShapeObject()).getFill()){
					if (!type.equals("Line")) {
					...
					if (!type.equals("Line")) {
                        fill.setBackground(Color.black);
						((Shape2D) panel.getCurrentShapeObject()).setFill(false);
					} else if (type.equals("Line")) {
						panel.getCurrentShapeObject().setColor(toFill);
					}
				}
				...
				if (fileChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
					try {
						...
						("eg.edu.alexu.csd.oop.paintModel." + className);
						@SuppressWarnings("unused")
					  ...
					  ("You can't select this File", JOptionPane.ERROR_MESSAGE);    
