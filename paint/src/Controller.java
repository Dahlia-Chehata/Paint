...
public class Controller extends JPanel {
	...
	public Controller(final JLabel statusLabel) {
		...
		enable = false;
		afterMoving = false;
		Shape.undoStack.push(new ArrayList<Shape>());
		afterResizing = false;
		selected = new Point();
		currentState = 0;
 ...
 */
	public final void addTriangle(Class<?> loadedClass) {
		addedTriangle = loadedClass.getConstructors()[1];
	}
/**
 ...
 */
	...
	(new Point(eventx, eventy), new Point(eventx, eventy), currentShapeColor);
			break;
...
(new Point(eventx, eventy), new Point(eventx, eventy), currentShapeColor,
		...
		case Constants.ovalCode:
			currentShapeObject = new Oval(new Point(eventx, eventy),
					new Point(eventx, eventy), currentShapeColor,
					currentShapeFilled);
			break;
		case Constants.trianlgeCode:
			try {
				currentShapeObject =
	(Shape2D) addedTriangle.newInstance(new Point(eventx, eventy),
	...
	currentShapeObject = (Shape2D) addedSquare.newInstance(
	new Point(eventx, eventy),new Point(eventx, eventy), currentShapeColor,
	currentShapeFilled);
	...
	public void toUndoStack() {
		...
		for (Shape o : Shape.myShapes) {
			...
			if (type.equals("Line")) {
				toBeAdded.add(new Line(new Point(o.getOneX(),
			o.getOneY()), new Point(o.getTwoX(), o.getTwoY()),
						Color.black));
			} else if (type.equals("Rectangle")) {
				toBeAdded.add(new Rectangle(new Point
		(o.getOneX(), o.getOneY()), new Point(o.getTwoX(), o.getTwoY()),
					o.getColor(), ((Shape2D) o).getFill()));
			} else if (type.equals("Square")) {
				try {
					toBeAdded.add((Shape2D) addedSquare.newInstance
					(new Point(o.getOneX(), o.getOneY()),
				new Point(o.getTwoX(), o.getTwoY()), o.getColor(), 
				((Shape2D) o).getFill()));
				} catch (Exception e) {
				}
			} else if (type.equals("Oval")) {
				toBeAdded.add(new Oval(new Point(o.getOneX(),
				o.getOneY()), new Point(o.getTwoX(), o.getTwoY()),
						o.getColor(), ((Shape2D) o).getFill()));
			} else if (type.equals("Circle")) {
				toBeAdded.add(new Circle(new Point(o.getOneX(),
				o.getOneY()), new Point(o.getTwoX(), o.getTwoY()),
					o.getColor(), ((Shape2D) o).getFill()));
			} else if (type.equals("Triangle")) {
				try {
					toBeAdded.add((Shape2D) addedTriangle.newInstance(
			new Point(o.getOneX(), o.getOneY()),
	...
	private class MouseHandler extends MouseAdapter {
		...
		int ret = o.response(new Point(event.getX(), event.getY()));
				...
				if (currentShapeObject != null || movableNumber != -1) {
	...
	("Mouse Coordinates X: %d Y: %d", event.getX(), event.getY()));
		}
	}
}
